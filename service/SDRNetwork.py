...
一维时候的simulation
多维simulation的代码
构建test函数，看看过拟合的情况。看看模型的复杂度到底处在什么水平。
用每一层（比如w1）的频率分布图来visualize参数的变化。挺好。画均值标准差，也不错，求和感觉有点太笨了。or plot unit值也可以
gamma的图何不如画在一张图里。
...
检查事项：1.检查对gamma导数  2.检查gamma更迭事项
3.如果控制gamma初值为(0,0,0,1)，迭代，最后能收敛成什么样？
4.如果控制gamma为(0,0,0,1)，且不更新（就是一个单纯的神经网络，第四维度被降掉），能收敛成什么样。
...
reduction=sum,liekai

看看怎么样的初值（x的初值或者gamma的初值）  会导致不收敛  （不收敛体现为gamma模长大于1，即gamma与h不正交）。

h*t不正交应该是数值计算造成的，算法没问题，解决方法：考虑强制归一化
...
markdown打公式，放大倍数然后截屏
R xtable 生成latex表格代码
mathtype：word，能够识别latex代码  【先用latex检查latex代码】
...
在 03-14-check中，
不强制给r归一：30次试验有12次失败。
强制给r归一后：30次试验有1次失败。是20-14-00文件。我觉得是r的初值太小，我认为可以针对这个r的初值太小，再做一些测试。
